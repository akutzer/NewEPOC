...
def test_wsidir_write_permissions(wsi_dir: Path):
    try:
        ...
        logging.warning("No write permissions for wsi directory! If multiple tissueMAP processes are running "
...
def preprocess(output_dir: Path, wsi_dir: Path, extractor_path: Path, classifier_path: Path, cache_dir: Path,
    ...
    if feat_extractor == "ctp":
        extractor = FeatureExtractor.init_ctranspath(extractor_path, device)
    elif feat_extractor == "uni":
        extractor = FeatureExtractor.init_uni(extractor_path, device)
    else:
        raise Exception(f"Invalid feature extractor '{feat_extractor}' selected")
    patch_classifier = HistoClassifier.from_pretrained(classifier_path, device=device)

    # Create cache and output directories
    if cache:
        cache_dir.mkdir(exist_ok=True, parents=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    norm_method = "tissueMAP_macenko_" if norm else "tissueMAP_raw_"
